...
class ProfilingService:
    ...
    async def _validate_diet(self, diet: str | None) -> None:
        if not diet:
            return
        from app.core.settings_model import SystemSetting, DEFAULTS
        result = await self.db.execute(select(SystemSetting).where(SystemSetting.key == "dietary_multipliers"))
        setting = result.scalar_one_or_none()
        valid_diets = list(setting.value.keys()) if setting else list(DEFAULTS["dietary_multipliers"].keys())
        if diet not in valid_diets:
            raise ValueError(f"Invalid dietary preference. Allowed: {valid_diets}")

    async def add_member(self, user_id: uuid.UUID, data: MemberCreate) -> Member:
        household = await self.get_my_household(user_id)
        await self._validate_diet(data.dietary_preference)
